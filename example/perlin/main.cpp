  ...
  void loop() {
    ...
    for (uint8_t x = 0; x < BUF_W; x++) {
      for (uint8_t y = 0; y < BUF_H; y++) {
        float n1 = sn.fractal(5, x / 240.0, y / 240.0);  // sn.noise returns [-1,1]
        float n2 = sn.noise(x, y);                       // sn.noise returns [-1,1]
        ...
        if (n1 < -.1) {  // deep water
          color =
              rgb565(72 * (1 + n1) + mix(5, n1, n2), 72 * (1 + n1) + mix(5, n1, n2), 190 * (1 + n1) + mix(5, n1, n2));
          ...
          color = rgb565(35, 57, 7) - rgb565(mix(10, n1, n2), mix(20, n1, n2), mix(10, n1, n2));
        } else {
