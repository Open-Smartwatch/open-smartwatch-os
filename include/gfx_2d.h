...
class Graphics2D {
  public:
    ...
    uint16_t* getChunk(uint8_t chunkId) {
        return buffer[chunkId];
    }
    uint8_t getChunkHeight() {
        return chunkHeight;
    }
    uint16_t getChunkOffset(uint8_t chunkId) {
        return isRound ? chunkXOffsets[chunkId] : 0;
    }
    uint16_t getChunkWidth(uint8_t chunkId) {
    ...
    void setMissingPixelColor(uint16_t color) {
        missingPixelColor = color;
    }
    uint16_t getMissingPixelColor(void) {
