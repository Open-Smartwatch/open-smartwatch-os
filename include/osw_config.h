...
#define _OSW_CONFIG_GETTER(T, F) inline T F(const char* id, T def) {return this->prefs.F(id, def);}
#define _OSW_CONFIG_SETTER(T, F) inline void F(const char* id, T value) {if(this->readOnly) return; this->prefs.F(id, value);}
