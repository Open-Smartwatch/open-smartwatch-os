...
bool OswHal::hasGPSFix(void) {
    return fix.latitude() != 0 || fix.longitude() != 0;
}
gps_fix* OswHal::gpsFix(void) {
    return &fix;
}
double OswHal::gpsLat(void) {
    return fix.latitude();
}
double OswHal::gpsLon(void) {
    return fix.longitude();
}

bool OswHal::isDebugGPS() {
    return _debugGPS;
}
void OswHal::setDebugGPS(bool debugGPS) {
    _debugGPS = debugGPS;
}

void OswHal::gpsForceOn(boolean on) {
    digitalWrite(GPS_FON, on ? HIGH : LOW);
}

uint8_t OswHal::gpsNumSatellites(void) {
