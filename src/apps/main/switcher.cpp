...
void OswAppSwitcher::loop() {
    ...
    if (_enableSleep and hal->btnIsDownFor(_btn) > this->_timeForLongPress + this->_timeForSleepPress) {
    ...
    switch (_type) {
    case LONG_PRESS:
        if (hal->btnIsDownFor(_btn) > this->_timeForLongPress) {
            _doSwitch = true;
        }
        break;
    case SHORT_PRESS:
    ...
    if (hal->btnIsDown(_btn)) {
        ...
        case LONG_PRESS:
            ...
            if (hal->btnIsDownFor(_btn) > this->_timeForLongPress) {
                ...
                uint8_t progress = 0;
                if (hal->btnIsDownFor(_btn) > this->_timeForLongPress / 2) {
                    progress = (hal->btnIsDownFor(_btn) - (this->_timeForLongPress / 2)) /
