...
#define COLOR_STEP rgb565(38, 70, 83)
#define COLOR_BLACK rgb565(0, 0, 0)
#define COLOR_WHITE rgb565(255, 255, 255)

void OswAppWatchfaceBinary::drawWatch(OswHal* hal, Graphics2D* gfx2d, const uint16_t& primaryColor) {
  uint32_t second = 0;
  uint32_t minute = 0;
  uint32_t hour = 0;
  ...
  //cosmetic
  hal->gfx()->drawLine(width /2 + 16, height / 8 * 1, width /2 + 16, height / 8 * 7, COLOR_WHITE);
  
  //steps
  uint32_t steps = hal->getStepCount();
  hal->gfx()->setTextColor(COLOR_STEP,COLOR_BLACK);
  hal->gfx()->setTextCursor(width /2 + 16, height / 8 * 1);
